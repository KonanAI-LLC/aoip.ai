...
if flock -n 200;then
  if [[ ! -d "${DATA_BASE}/${SRC_EVAL_PATTERN}" ]];then
  ...
  while [[ ! -d "${DATA_BASE}/${SRC_EVAL_PATTERN}"  ]]; do
...
aws s3 cp --profile "${PROFILE}" --quiet s3://"${BUCKET}/${SPLIT_S3_PREFIX}/${file_id}" \
"${DATA_BASE}/${SPLIT_DIR_PATTERN}/${file_id}" --recursive
